...
> = {
  purple: { dot: 'bg-purple-600', hoverText: 'hover:text-purple-600', pillBg: 'bg-purple-50', pillText: 'text-purple-600' },
  sky: { dot: 'bg-sky-400', hoverText: 'hover:text-sky-400', pillBg: 'bg-sky-50', pillText: 'text-sky-600' },
  emerald: { dot: 'bg-emerald-600', hoverText: 'hover:text-emerald-600', pillBg: 'bg-emerald-50', pillText: 'text-emerald-600' },
};
...
function formatEventRange(start: Date, end: Date) {
  const datePart = start.toLocaleDateString(undefined, { month: 'short', day: '2-digit', year: 'numeric' });
...
function isSameMonth(a: Date, monthDate: Date) {
  return a.getFullYear() === monthDate.getFullYear() && a.getMonth() === monthDate.getMonth();
}
...
// day is "booked" if any booked event on that day
...
export default function Navbar() {
  ...
  const [activeMonth, setActiveMonth] = useState<Date>(() => startOfMonth(new Date(2024, 0, 1)));
  ...
  function editEvent(id: string) {
    setEvents((prev) =>
      prev.map((e) => (e.id === id ? { ...e, title: e.title.includes('(Edited)') ? e.title : `${e.title} (Edited)` } : e)),
    );
    setOpenMenuForId(null);
  }

  function eventForDay(day: Date) {
    const dayEvents = events.filter((e) => sameDay(e.start, day)).slice().sort(sortByStart);
  ...
  // ✅ NEW: check if the chosen booking window (duration) is available
        ...
        <div className="w-full max-w-7xl mx-auto px-2 lg:px-8">
          <div className="grid grid-cols-12 gap-8 max-w-4xl mx-auto xl:max-w-full">
            ...
            <div className="col-span-12 xl:col-span-5">
              <h2 className="font-manrope text-3xl leading-tight text-gray-900 mb-1.5">Buchen</h2>
              <p className="text-lg font-normal text-gray-600 mb-6">
                Ausgewählte Zeit: {selectedDate.toLocaleDateString()} · {pad2(selectedHour)}:00–
              ...
              {/* ✅ NEW: Booking form (instead of showing events) */}
              <div className="p-6 rounded-xl bg-white">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-base font-semibold text-gray-900">Neue Buchung</h3>
                ...
                {isBookingWindowFree ? (
                  <div className="mb-4 p-3 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-700">
                    ...
                    <div className="text-xs mt-1">Du kannst diese Zeit buchen.</div>
                  </div>
                ) : (
                  <div className="mb-4 p-3 rounded-lg bg-red-50 border border-red-200 text-red-700">
                    ...
                    {blockingEvent ? (
                      <div className="text-xs mt-1">
                        Blockiert durch: <span className="font-medium">{blockingEvent.title}</span> ·{' '}
                        {formatTime(blockingEvent.start)}–{formatTime(blockingEvent.end)}
                      </div>
                    ) : null}
                  </div>
                )}
                ...
                <div className="mt-4 flex gap-2">
                  <button
                    ...
                    disabled={!isBookingWindowFree || !bookingName.trim() || !bookingLocation.trim()}
                    className={[
                      'px-4 py-2 rounded-lg text-sm font-semibold transition-all',
                      !isBookingWindowFree || !bookingName.trim() || !bookingLocation.trim()
              ...
              {/* Keep your upcoming list (optional) */}
              <div className="mt-6">
                <p className="text-lg font-normal text-gray-600 mb-4">Nächste Termine</p>
                <div className="flex gap-5 flex-col">
                  {upcomingEvents.length === 0 ? (
                    <div className="p-6 rounded-xl bg-white">
                      <p className="text-base font-medium text-gray-900">Frei</p>
                      <p className="text-base font-normal text-gray-600">Keine Termine.</p>
                    </div>
                  ) : (
                        ...
                        <div key={ev.id} className="p-6 rounded-xl bg-white">
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <div className="relative inline-flex">
                              ...
                              {openMenuForId === ev.id && (
                                <div className="rounded-xl shadow-lg bg-white absolute top-full -left-10 w-max mt-2" role="menu">
                                  <ul className="py-2">
                                    <li>
                                      <button
                          ...
                          <h6 className="text-xl leading-8 font-semibold text-black mb-1">{ev.title}</h6>
                          <p className="text-base font-normal text-gray-600">
                            {ev.location ? <span className="block text-sm text-gray-500">📍 {ev.location}</span> : null}
                            {ev.bookingType ? <span className="block text-sm text-gray-500">🏷️ {ev.bookingType}</span> : null}
                            {ev.description ?? '—'}
                          </p>
                        </div>
                      );
            ...
            <div className="col-span-12 xl:col-span-7 px-2.5 py-5 sm:p-8 bg-gradient-to-b from-white/25 to-white xl:bg-white rounded-2xl max-xl:row-start-1">
              <div className="flex flex-col md:flex-row gap-4 items-center justify-between mb-5">
                <div className="flex items-center gap-4">
                  <h5 className="text-xl leading-8 font-semibold text-gray-900">{monthTitle(activeMonth)}</h5>
                  ...
                  {monthDays.map((day) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {ev && (
                          <div className={['absolute top-9 bottom-1 left-3.5 p-1.5 xl:px-2.5 h-max rounded', COLOR_STYLES[ev.color].pillBg].join(' ')}>
                            <p className={['hidden xl:block text-xs font-medium mb-px whitespace-nowrap', COLOR_STYLES[ev.color].pillText].join(' ')}>
                              {ev.title}
                            </p>
                            <span className={['hidden xl:block text-xs font-normal whitespace-nowrap', COLOR_STYLES[ev.color].pillText].join(' ')}>
              ...
              <div className="mt-5 text-sm text-gray-600">
                <span className="font-medium text-gray-900">Selected:</span> {selectedDate.toLocaleDateString()} ·{' '}
                <span className="font-medium text-gray-900">Hour:</span> {pad2(selectedHour)}:00
              </div>
            </div>
          </div>
        </div>
